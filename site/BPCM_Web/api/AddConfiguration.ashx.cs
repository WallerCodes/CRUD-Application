...
public class AddConfiguration : AjaxHandler
{
    ...
    public override object ProcessRequest(HttpSessionState session, HttpRequest request, DUser dUser, HttpContext context)
    {
        ...
        bool isCopy = parseSingleParameter<bool>(request, "isCopy", true);
        ...
        try
        {
            result = DBTools.AddConfiguration(
                application,
                language,
                dnis,
                destination,
        ...
        if (isCopy)
        {
            DBTools.UserAudit(user, user, "", "Copy Configuration", UserAuditAction.COPY_CONFIGURATION);
        }
        else
        {
            DBTools.UserAudit(user, user, "", "Create Configuration", UserAuditAction.CREATE_CONFIGURATION);
        }
