...
function getCalleeName(node) {
	...
	if (callee.type === AST_NODE_TYPES.MemberExpression && !callee.computed && isIdentifier(callee.property)) return callee.property.name;
	return null;
}
/**
...
function getPropertyName(property, effort = "min") {
	...
	if (isIdentifier(key) && !property.computed) return key.name;
...
function getIdentifierAt(node, position) {
	...
	while (current.type === AST_NODE_TYPES.MemberExpression) {
		...
		identifiers.unshift(isIdentifier(property) ? property : null);
		current = unwrap(current.object);
	}
	identifiers.unshift(isIdentifier(current) ? current : null);
...
const isEqual = dual(2, (a, b) => {
	...
	switch (true) {
		...
		case isIdentifier(a) && isIdentifier(b): return a.name === b.name;
...
var traverse_exports = /* @__PURE__ */ __exportAll({
	findEnclosingTryBlock: () => findEnclosingTryBlock,
...
*/
function findEnclosingTryBlock(node) {
	const parent = node.parent;
	if (parent == null || parent.type === AST_NODE_TYPES.Program) return null;
	if (parent.type === AST_NODE_TYPES.TryStatement && node === parent.block) return parent;
