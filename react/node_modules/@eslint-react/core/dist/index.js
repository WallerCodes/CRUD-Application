...
function isAPI(api) {
	...
	function dual(context, ...rest) {
		return rest.length === 1 ? func(context, rest[0]) : (node) => func(context, node);
	}
	return dual;
}
/**
...
function isClassComponent(node) {
	if ("superClass" in node && node.superClass != null) {
		...
		switch (true) {
			...
			case node.superClass.type === AST_NODE_TYPES.MemberExpression && Check.isIdentifier(node.superClass.property): return re.test(node.superClass.property.name);
		}
	}
	return false;
}
...
function createLifecycleChecker(methodName, isStatic = false) {
	return (node) => Check.isPropertyOrMethod(node) && node.static === isStatic && Check.isIdentifier(node.key) && node.key.name === methodName;
}
...
function isAssignmentToThisState(node) {
	...
	while (current.type === AST_NODE_TYPES.MemberExpression) {
		...
		if (object.type === AST_NODE_TYPES.ThisExpression && Check.isIdentifier(property, "state")) return true;
		current = object;
	}
	return false;
}
...
function getCreateElementPropsObject(context, node) {
	...
	return propsObject.type === AST_NODE_TYPES.ObjectExpression ? propsObject : null;
}
/**
...
function isFunctionWithLooseComponentName(context, fn, allowNone = false) {
	...
	if (Check.isIdentifier(id)) return isFunctionComponentNameLoose(id.name);
...
function isFunctionComponentDefinition(context, node, hint) {
	...
	const [parentCallee, parentCalleeName] = parent.type === AST_NODE_TYPES.CallExpression ? [Extract.unwrap(parent.callee), Extract.getCalleeName(parent)] : [null, null];
	switch (true) {
		...
		case parentCallee != null && parentCallee.type === AST_NODE_TYPES.MemberExpression && parentCalleeName === "flatMap":
			...
			break;
		case parent.type === AST_NODE_TYPES.CallExpression && getFunctionId(node) == null && !isFunctionComponentWrapperCall(context, parent) && !isCreateElementCall(context, parent): if (hint & FunctionComponentDetectionHint.DoNotIncludeFunctionDefinedAsArbitraryCallExpressionCallback) return false;
	}
